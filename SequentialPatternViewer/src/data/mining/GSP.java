...
public class GSP {
    ...
    public void getOption(LinkedList<LinkedList<String>> sequences, int option){
        if(option == 4){
...
//            this.recoveredPatternList.addAll(recoveredPatterns);
    ...
    public LinkedList<LinkedList<String>> combination(LinkedList<LinkedList<String>> frequentPatterns) {
        ...
        while(patternIterator.hasNext()){
            sequencePatterns.addAll(findSequencePatterns(patternIterator.next(), frequentPatterns));
        }
    ...
    private boolean containsSubSequence(LinkedList<String> currentAplicant, LinkedList<String> sequence) {
    ...
    private LinkedList<LinkedList<String>> findSequencePatterns(LinkedList<String> frequentPatterns1, LinkedList<LinkedList<String>> frequentPatterns2) {
    ...
    private LinkedList<LinkedList<String>> prune(LinkedList<LinkedList<String>> sequencePatterns, LinkedList<LinkedList<String>> pruneList) {
        Iterator<LinkedList<String>> prune = pruneList.iterator();
        while(prune.hasNext()){sequencePatterns.remove(prune.next());}
        return sequencePatterns;
    }

    private LinkedList<LinkedList<String>> pruneFirstSequencePatterns(LinkedList<String> patternsC1, LinkedList<String> pruneList) {
     ...
     * @param sequences
     */
    private void getPrefix(LinkedList<LinkedList<String>> sequences) {
    ...
    private LinkedList<String> prefix(LinkedList<String> sequence){
        LinkedList<String> seqList = new LinkedList<String>();
            for(int j=0; j<sequence.size(); j++){
                ...
                for(int k=0; k<seq[1].length(); k++){
                    if(!Character.isUpperCase(seq[1].charAt(k))) index = k;
                    else if(prefixSize>1) {prefixSize--; index=k;}
                    else break;
                }
                if(index>0) seqList.add(seq[1].substring(0, index+1));
            }
        return seqList;
    }
...
//            this.sequenceList.add(seqList);
    ...
    private LinkedList<String> suffix(LinkedList<String> sequence){
        LinkedList<String> seqList = new LinkedList<String>();
            for(int j=0; j<sequence.size(); j++){
                ...
                int prefixSize = this.size;
                String[] seq = sequence.get(j).split("-");
                for(int k=seq[1].length()-1; k>=0; k--){
                    if(!Character.isUpperCase(seq[1].charAt(k))){
                       index = k;
                    }else if(prefixSize>1) {prefixSize--; index=k;}
                    else break;
                }
                if(index>0) seqList.add(seq[1].substring(index-1,  seq[1].length()));
                else if(index == 0) seqList.add(seq[1]);
            }
        return seqList;
    }
    ...
    private void getEqualMethod(LinkedList<LinkedList<String>> sequences) {
        this.sequenceList = new LinkedList<LinkedList<String>>();
        for(int i=0; i<sequences.size(); i++){
            LinkedList<String> sequence = sequences.get(i);
            LinkedList<String> seqList = new LinkedList<String>();
